...
#define LL_EFUSE_BANK_COUNT      (3UL)
...
/**
 * @brief Configure eFuse timing parameters (TCR.PGM / TCR.EXTEND).
 * @param pgm     Program pulse width (12 bits).
 * @param extend  Extended program pulse width (4 bits).
 */
static inline void ll_efuse_set_timing(EFUSEC_TypeDef *efuse, uint32_t pgm, uint32_t extend)
{
	WRITE_REG(efuse->TCR,
		  MAKE_REG_VAL(pgm, EFUSEC_TCR_PGM_Msk, EFUSEC_TCR_PGM_Pos) |
		  MAKE_REG_VAL(extend, EFUSEC_TCR_EXTEND_Msk, EFUSEC_TCR_EXTEND_Pos));
}
...
/** @brief Set the debug select (RSVDR.DBG_SEL). */
static inline void ll_efuse_set_dbg_sel(EFUSEC_TypeDef *efuse, uint32_t sel)
{
	MODIFY_REG(efuse->RSVDR, EFUSEC_RSVDR_DBG_SEL,
		   MAKE_REG_VAL(sel, EFUSEC_RSVDR_DBG_SEL_Msk, EFUSEC_RSVDR_DBG_SEL_Pos));
}

/** @brief Set the reserve 1 field (RSVDR.RESERVE1). */
static inline void ll_efuse_set_reserve1(EFUSEC_TypeDef *efuse, uint32_t val)
{
	MODIFY_REG(efuse->RSVDR, EFUSEC_RSVDR_RESERVE1,
		   MAKE_REG_VAL(val, EFUSEC_RSVDR_RESERVE1_Msk, EFUSEC_RSVDR_RESERVE1_Pos));
}

/** @brief Set the reserve 0 field (RSVDR.RESERVE0). */
static inline void ll_efuse_set_reserve0(EFUSEC_TypeDef *efuse, uint32_t val)
{
	MODIFY_REG(efuse->RSVDR, EFUSEC_RSVDR_RESERVE0,
		   MAKE_REG_VAL(val, EFUSEC_RSVDR_RESERVE0_Msk, EFUSEC_RSVDR_RESERVE0_Pos));
}
